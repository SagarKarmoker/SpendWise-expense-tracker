  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {/* Total Balance Card */}
        <div className="bg-accent-black dark:bg-surface-dark rounded-3xl p-6 text-white relative overflow-hidden shadow-xl">
          ...
          <div className="relative z-10">
            ...
            <p className={`text-sm mt-2 ${balance >= 0 ? 'text-primary' : 'text-red-400'}`}>
        ...
        <div className="bg-surface-light dark:bg-surface-dark rounded-3xl p-6 shadow-soft">
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-sm text-subtext-light dark:text-subtext-dark">Total Income</span>
          </div>
          <p className="text-3xl font-bold text-text-light dark:text-text-dark">
            ৳{income.toFixed(2)}
          </p>
          {lastMonthIncome > 0 || thisMonthIncome > 0 ? (
            <p className={`text-sm mt-2 ${incomeChange >= 0 ? 'text-green-600' : 'text-red-500'}`}>
              {incomeChange >= 0 ? '+' : ''}{incomeChange}% vs last month
            </p>
          ) : (
            <p className="text-sm mt-2 text-subtext-light dark:text-subtext-dark">No data yet</p>
          )}
        </div>
        ...
        <div className="bg-surface-light dark:bg-surface-dark rounded-3xl p-6 shadow-soft">
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-sm text-subtext-light dark:text-subtext-dark">Total Expenses</span>
          </div>
          <p className="text-3xl font-bold text-text-light dark:text-text-dark">
            ৳{expense.toFixed(2)}
          </p>
          {lastMonthExpense > 0 || thisMonthExpense > 0 ? (
            <p className={`text-sm mt-2 ${expenseChange <= 0 ? 'text-green-600' : 'text-red-500'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {chartData.length > 0 ? (
          ...
          <div className="bg-surface-light dark:bg-surface-dark rounded-3xl p-6 shadow-soft flex items-center justify-center">
            ...
            <div className="h-52">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={sourceChartData} layout="vertical" margin={{ left: 20 }}>
                  ...
                  <XAxis type="number" tickFormatter={(v) => `৳${v}`} fontSize={12} stroke="#888" />
                  <YAxis type="category" dataKey="source" fontSize={12} stroke="#888" width={80} />
                  <Tooltip
                    contentStyle={{
                      backgroundColor: '#1A1A1A',
                      border: 'none',
                  ...
                  />
                  <Bar dataKey="amount" fill="#D4F00C" radius={[0, 8, 8, 0]} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
      ...
      <div className="bg-surface-light dark:bg-surface-dark rounded-3xl p-6 shadow-soft">
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-lg font-bold text-text-light dark:text-text-dark">Recent Transactions</h3>
        ...
        {allTransactions.length === 0 ? (
          <div className="text-center py-8">
            <p className="text-subtext-light dark:text-subtext-dark">No transactions yet.</p>
            <button
              onClick={() => navigate('/transactions?add=true')}
          ...
          <div className="space-y-4">
            {allTransactions.slice(0, 5).map((transaction) => {
              ...
              return (
                <div
                  key={transaction.id}
                  className="flex items-center justify-between p-4 bg-accent-gray dark:bg-background-dark rounded-2xl"
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-subtext-light dark:text-subtext-dark flex items-center gap-1.5">
                        {new Date(transaction.date).toLocaleDateString()}
                        {transaction.category && ` • ${transaction.category.name}`}
                        <span className="inline-flex items-center gap-1 ml-1 px-1.5 py-0.5 rounded-full bg-gray-100 dark:bg-gray-800 text-[10px]">
                          <SourceIcon className="w-2.5 h-2.5" />
                          {sourceInfo?.label || 'Cash'}
                        </span>
                      </p>
                    </div>
                  </div>
